...
const ORDER_TABLE = "Order-zspcwwlagvhmnf5y7ow4ej5tku-dev";
const ORDER_TYPE = "Order";
const MERCANCIA_ORDER_TABLE = "MercanciaOrder-zspcwwlagvhmnf5y7ow4ej5tku-dev";
const MERCANCIA_ORDER_TYPE = "MercanciaOrder";
...
const createMercanciaOrder = async (payload) => {
  let mercanciaOrders = [];
  for (let i = 0; i < payload.cart.length; i++) {
    const cartItem = payload.cart[i];
    mercanciaOrders.push({
      PutRequest: {
        Item: {
          id: uuidv4(),
          __typename: MERCANCIA_ORDER_TYPE,
          mercancia_id: cartItem.id,
  ...
  params["RequestItems"][MERCANCIA_ORDER_TABLE] = mercanciaOrders;
  console.log(params);
...
exports.handler = async (event) => {
  try {
    ...
    await createMercanciaOrder(payload);
